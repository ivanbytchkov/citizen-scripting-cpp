...
extern "C"
{
    ...
    __attribute__((import_module("fxcpp"), import_name("schedule_bookmark")))
    void __fxcpp_schedule_bookmark(int32_t bookmark_id, int32_t deadline_ms);

    __attribute__((import_module("fxcpp"), import_name("is_manifest_version_v2_between")))
    int32_t __fxcpp_is_manifest_version_v2_between(const char* lower, uint32_t lower_len, const char* upper, uint32_t upper_len);
}
...
struct EventHandlerEntry { int32_t id; fx::EventHandler handler; };
...
struct Context
{
...
#if __cpp_exceptions
            try { cb(); }
            catch (const std::exception& e) { fprintf(stderr, "[script:%s] Unhandled exception in timer %d: %s\n", resourceName.c_str(), id, e.what()); }
            catch (...) { fprintf(stderr, "[script:%s] Unhandled non-standard exception in timer %d\n", resourceName.c_str(), id); }
#else
            cb();
#endif
        }
...
#if __cpp_exceptions
            try { h(); }
            catch (const std::exception& e) { fprintf(stderr, "[script:%s] Unhandled exception in tick handler: %s\n", resourceName.c_str(), e.what()); }
            catch (...) { fprintf(stderr, "[script:%s] Unhandled non-standard exception in tick handler\n", resourceName.c_str()); }
#else
            h();
#endif
        }
    ...
    void dispatchEvent(const char* name, uint32_t nameLen, const uint8_t* args, uint32_t argsLen, const char* src, uint32_t srcLen)
    {
        ...
        {
#if __cpp_exceptions
            try { entry.handler(srcStr, ea); }
            catch (const std::exception& e) { fprintf(stderr, "[script:%s] Unhandled exception in event '%s': %s\n", resourceName.c_str(), key.c_str(), e.what()); }
            catch (...) { fprintf(stderr, "[script:%s] Unhandled non-standard exception in event '%s'\n", resourceName.c_str(), key.c_str()); }
#else
            entry.handler(srcStr, ea);
#endif
            if (__fxcpp_was_event_canceled()) break;
        }
    }
};
...
inline void removeEventHandler(int32_t token)
{
    ...
    if (mapIt == c->handlerEventMap.end()) return;
    std::string event = mapIt->second;
    c->handlerEventMap.erase(mapIt);
    auto it = c->events.find(event);
    if (it == c->events.end()) return;
    auto& vec = it->second;
    vec.erase(std::remove_if(vec.begin(), vec.end(), [token](const fxw_internal::EventHandlerEntry& e) { return e.id == token; }), vec.end());
}
...
inline void emitLatent(const std::string& event, int target, int bps, TArgs&&... vals)
{
    ...
    invokeNative(HashString("TRIGGER_LATENT_CLIENT_EVENT_INTERNAL"), {
        NativeArg::ptr(event.c_str()),
        NativeArg::ptr(targetStr.c_str()),
        NativeArg::ptr(w.buf.data()),
        NativeArg(static_cast<int32_t>(w.buf.size())),
        NativeArg(static_cast<int32_t>(bps))
    });
}
...
inline void performHttpRequest(const std::string& url, HttpCallback cb, const std::string& method = "GET", const std::string& data = "", const std::string& headers = "{}")
{
    ...
    if (!s_registered)
    {
        ...
        on("__cfx_internal:httpResponse", [](const std::string&, const EventArgs& args) {
            ...
            if (it == s_httpCallbacks.end()) return;
...
#if __cpp_exceptions
            try { userCb(status, body, respHeaders); }
            catch (...) {}
#else
...
inline void resumeCoroutineById(uint64_t id)
{
    ...
    if (handle.done())
    {
        ...
        if (handle.promise().exception)
        {
            ...
            catch (const std::exception& e) { fprintf(stderr, "[script:%s] Unhandled exception in thread: %s\n", rn, e.what()); }
            catch (...) { fprintf(stderr, "[script:%s] Unhandled non-standard exception in thread\n", rn); }
        }
#endif
        handle.destroy();
        coros.erase(it);
...
FXCPP_WASM_EXPORT(fxcpp_tick_bookmarks) void fxcpp_tick_bookmarks(int32_t* bookmarks, int32_t count)
{
